...
#else
#define MMI_SLK_SET_SELECTION_DEFAULT MMI_SLK_SET_SELECTION_30_SEC//MMI_SLK_SET_SELECTION_OFF
#endif
...
static mmi_slk_set_selection_enum mmi_slk_set_get_selection(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ReadValue(NVRAM_SETTING_AUTOKEYPADLOCK_TIME, &nvram_data, DS_BYTE, &error);
    if (error != NVRAM_READ_SUCCESS ||
        nvram_data >= MMI_SLK_SET_SELECTION_END_OF_ENUM)
    {
        nvram_data = (U8)MMI_SLK_SET_SELECTION_DEFAULT;
        WriteValue(NVRAM_SETTING_AUTOKEYPADLOCK_TIME, &nvram_data, DS_BYTE, &error);
    }
    ...
    switch (evt->evt_id)
    {
        case EVT_ID_CUI_MENU_LIST_ENTRY:
            if (menu_evt->parent_menu_id == MENU_ID_SECSET_MAIN_MENU)
            {
                ...
                if (mmi_slk_set_get_selection() == MMI_SLK_SET_SELECTION_OFF)
                {
                ...
                cui_menu_set_item_hint(
                    menu_evt->sender_id,
                    MENU_ID_SLK_SETTING_ENTRY,
                    get_string(hint_str));
            }
            break;

        case EVT_ID_CUI_MENU_ITEM_SELECT:
            if (menu_evt->highlighted_menu_id == MENU_ID_SLK_SETTING_ENTRY)
            {
                ...
                mmi_frm_group_enter(GRP_ID_SLK_SETTING, MMI_FRM_NODE_SMART_CLOSE_FLAG);
