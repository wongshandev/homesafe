...
void CLAM_EINT_HISR(void)
{
	...
	{
		//neodbg(MOD_MED,"LINE(%d)FILE(%s)clam_state == LEVEL_LOW",__LINE__,__FILE__);	
	
		kal_prompt_trace(MOD_WAP,"CLAM_EINT_HISR 1");
		...
		EINT_SW_Debounce_Modify(CLAMDET_EINT_NO,PLUGIN_DEBOUNCE_TIME);								
	...
	{
		kal_prompt_trace(MOD_WAP,"CLAM_EINT_HISR 2");
		//neodbg(MOD_MED,"LINE(%d)FILE(%s)clam_state == LEVEL_HIGH",__LINE__,__FILE__);
		aux_id_data = (aux_id_struct*)
		construct_local_para(sizeof(aux_id_struct), TD_CTRL);

		aux_id_data->aux_id = AUX_ID_CLAM_OPEN;
		//srv_backlight_to_sleep_mode();
		//srv_backlight_turn_on(1);//add by hwy
		...
		DRV_BuildPrimitive(clam_ilm,
                            ...
                            MSG_ID_AUX_ID,       
                            aux_id_data);
		EINT_SW_Debounce_Modify(CLAMDET_EINT_NO,PLUGOUT_DEBOUNCE_TIME);
...
kal_bool Get_UART_Detection_Status(UART_PORT port)
{
   ...
   {
      case uart_port1:
         return (kal_bool)check_uart1;
//         break;
      case uart_port2:
         return (kal_bool)check_uart2;
//         break;
      default:
         return KAL_FALSE;
//         break;
