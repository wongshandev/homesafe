...
extern void AUX_Sendilm(module_type src_mod, module_type dst_mod, msg_type msgid, AUX_ID aux_id_no);
...
void ACCDET_setDebounceSet(kal_uint8 idx)
{
	if (idx>1)
		return;
	ACCDET_SetDebounce(accdet_state00, AccdetDebounce[idx][0]);  //Set debounce time
	ACCDET_SetDebounce(accdet_state01, AccdetDebounce[idx][1]);
	ACCDET_SetDebounce(accdet_state11, AccdetDebounce[idx][2]);
}
...
#if defined(__ACCDET_EINT_SUPPORT__)
   drv_trace4(TRACE_GROUP_3, ACCDET_EINT_STATUS_TRC,aux_eint_state,iA,iB,0);
#else
